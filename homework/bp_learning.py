...
for layer_idx, (n_neurons, name) in enumerate(zip(layer_sizes, layer_names)):
    ...
    for y_pos in y_positions:
        circle = plt.Circle((layer_idx * 0.4 + 0.2, y_pos), 0.04,
                           color='lightblue', ec='blue', linewidth=2)
        ax3.add_patch(circle)

    if n_neurons > max_display:
        ax3.text(layer_idx * 0.4 + 0.2, 0.08, f'...+{n_neurons - max_display}',
                ha='center', va='center', fontsize=9)

    ax3.text(layer_idx * 0.4 + 0.2, 0.95, name, ha='center', va='center',
    ...
    if layer_idx < len(layer_sizes) - 1:
        ...
        for y1 in y_positions:
            for y2 in next_y_positions:
                ax3.plot([layer_idx * 0.4 + 0.24, (layer_idx + 1) * 0.4 + 0.16],
                        [y1, y2], 'b-', alpha=0.1, linewidth=0.3)
